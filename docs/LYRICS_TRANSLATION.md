...
Rizumu can translate the currently displayed lyrics into the language selected
...
3. Rizumu resolves the active application locale and requests a translation.
4. If the detected source language matches the app language, the original lyrics
   remain visible and Rizumu explains that no translation is needed.
...
selected in Rizumu's language dialog. When the application locale list is
empty, Rizumu follows Android's effective configuration locale, matching the
app's normal “follow system” behaviour.

The translation therefore follows the language shown by Rizumu, not an
...
because word order and word count can change. For word-synced lyrics, Rizumu
...
Enabling Rizumu's **Reduce animations** preference turns it into an immediate
...
- `app/src/main/java/com/music/rizumu/data/lyrics/LyricsTranslation.kt`
  contains batching, source detection, timing reconstruction and bounded caching.
- `app/src/main/java/com/music/rizumu/ui/player/NowPlayingScreen.kt`
...
- `app/src/main/res/values-*/strings.xml` localizes the new UI messages for every
  language currently offered by Rizumu.

## Manual verification checklist

- Translate English lyrics with Rizumu set to Spanish.
...
- Use Spanish lyrics while Rizumu is set to Spanish and verify no translation
  view is created.
- Change Rizumu to another supported language and verify the new destination.
