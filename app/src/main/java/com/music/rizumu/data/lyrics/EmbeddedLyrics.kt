 ...
 *
 *  - `RIZUMU_LYRICS`, this app's own, holding the "enhanced" A2 form with the
 *    word timings intact — see [toEnhancedLrc]. Downloads saved by the app
 *    before its rename carry the same form under `BITCHORD_LYRICS`, and both
 *    spellings are searched so those files keep their word timings.
...
object EmbeddedLyrics {
    ...
    private fun ilstText(bytes: ByteArray, from: Int, endExclusive: Int, freeform: Boolean): String? {
        // The freeform item's name is the field itself, so both spellings have
        // to be searched: a file saved before the rename carries the old one.
        val markers = if (freeform) {
            WORD_LYRICS_FIELDS.map { it.toByteArray(Charsets.UTF_8) }
        } else {
            listOf(LYR_ATOM)
        }
        for (marker in markers) {
            ...
            while (true) {
                ...
                // The value is the first `data` box after the name, in both
                // layouts: a freeform item is mean/name/data, a standard one is
                // type/data.
    ...
    private fun vorbisComment(bytes: ByteArray, start: Int, end: Int): String? {
        ...
        repeat(count.coerceAtMost(4_096)) {
            ...
            if (name in WORD_LYRICS_FIELDS && value.isNotBlank()) return value
    ...
    private fun matroska(bytes: ByteArray): String? {
        var plain: String? = null
        for (name in WORD_LYRICS_FIELDS + "LYRICS") {
