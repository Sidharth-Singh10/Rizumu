...
object LocalMediaRepository {
    ...
    /**
     * Retrieves all songs in the `Music/Rizumu` directory, combining app downloads
    ...
    suspend fun getDownloadedSongs(context: Context): List<Song> = withContext(Dispatchers.IO) {
        ...
        runCatching {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                ...
                // Both folder names: files saved by the app under its old name
                // live in Music/BitChord and are still the user's downloads.
                for (folderName in listOf(DownloadStore.FOLDER, DownloadStore.LEGACY_FOLDER)) {
                    val folder = File(
        ...
        }.onFailure { Log.w(TAG, "Failed scanning the Music folders: ${it.message}") }
    ...
    suspend fun getLocalMusic(context: Context): List<Song> = withContext(Dispatchers.IO) {
        ...
        // whatever this app has downloaded into Music/Rizumu alongside
