...
import com.music.rizumu.data.sources.addon.AddonClient
import com.music.rizumu.data.sources.addon.AddonException
import com.music.rizumu.data.sources.addon.AddonNotFound
import com.music.rizumu.data.sources.addon.AddonStream
import com.music.rizumu.data.sources.addon.AddonTrack
import com.music.rizumu.playback.StreamContainer
...
class AddonSource(
     ...
     * [QualityUpgrade][com.music.rizumu.playback.QualityUpgrade] checks a
     ...
     * [TrackMatcher][com.music.rizumu.data.sources.TrackMatcher] decides which
    ...
    private fun openable(url: String, answer: AddonStream, trackId: String, tier: String): SourceStream? {
        ...
        if (answer.isEncrypted) {
            TrackLog.w(
                TAG,
                "${config.displayName}: $trackId came back encrypted, which Rizumu never asked for — passing",
            )
            return null
        }
