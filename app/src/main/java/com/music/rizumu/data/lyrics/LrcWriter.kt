 ...
 * `com.music.rizumu.download.LyricsTag`), and LRC is what that field is read
 ...
 * atom, where every other player looks, and this to a Rizumu-specific one
 ...
 */
internal const val WORD_LYRICS_FIELD = "RIZUMU_LYRICS"

/**
 * The field older downloads carry, from before the rename.
 *
 * Written into files saved by the app under its old name. Read but never
 * written, so a download made yesterday keeps its word timings without a
 * re-download — see [WORD_LYRICS_FIELDS].
 */
internal const val LEGACY_WORD_LYRICS_FIELD = "BITCHORD_LYRICS"

/** Both spellings, current first — what every reader looks for. */
internal val WORD_LYRICS_FIELDS = listOf(WORD_LYRICS_FIELD, LEGACY_WORD_LYRICS_FIELD)
