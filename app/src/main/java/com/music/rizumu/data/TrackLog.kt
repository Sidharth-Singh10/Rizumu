 ...
 * Rizumu to access all device logs?"* — which appears whenever the process
 ...
 * to logcat as well, so `adb logcat -s Rizumu` is unchanged.
 */
object TrackLog {
    ...
    private fun header(
        ...
        appendLine("Rizumu log — ${song.title} — ${song.artist}")
