package com.music.rizumu.data.settings
...
import com.music.rizumu.BuildConfig
import com.music.rizumu.auth.AuthStore
import com.music.rizumu.data.lyrics.LyricsSource
import com.music.rizumu.data.sources.SourceKind
import com.music.rizumu.playback.EqLayout
import com.music.rizumu.playback.EqualizerPreset
...
enum class AudioQuality(
     ...
     * [SourceConfig.enabled][com.music.rizumu.data.sources.SourceConfig.enabled],
 ...
 * Opus-in-WebM (see
 * [StreamResolver.resolveForDownload][com.music.rizumu.data.innertube.StreamResolver.resolveForDownload]),
...
object AppSettings {
    ...
    /**
     * Keep ordinary downloads in Music/Rizumu where other music apps can see
    ...
    // [SourceResolver.requestForNow][com.music.rizumu.data.sources.SourceResolver.requestForNow],
     ...
     * decode per track.
     *
     * See [com.music.rizumu.playback.smart.TransitionPlanner].
     */
     ...
     * [ModuleSource.unplayable][com.music.rizumu.data.sources.ModuleSource],
     * which is where the refusal is applied.
     *
     * Independent of whether the device *can* decode it — that question is
     * [DeviceCodecs.playsDolbyAtmos][com.music.rizumu.data.sources.DeviceCodecs],
    ...
    /**
     * Widens stereo output via [com.music.rizumu.playback.SpatialAudioProcessor],
     ...
     * all. See [CanvasRepository][com.music.rizumu.data.canvas.CanvasRepository].
     */
     ...
     * — see [CanvasCache][com.music.rizumu.data.canvas.CanvasCache] for why
     ...
     * [NowPlayingScreen][com.music.rizumu.ui.player.NowPlayingScreen].
     */
     ...
     * that have not gone away — see [ArtworkMesh][com.music.rizumu.ui.player.ArtworkMesh]
     * for the colour one (a cover that is nine-tenths black with a red stripe
     * comes back from the quantiser as a red screen) and
     * [ArtworkMeshBackdrop][com.music.rizumu.ui.player.ArtworkMeshBackdrop]
    ...
    /**
     * The order [lyricsSources] are asked in — see [LyricsRepository][com.music.rizumu.data.lyrics.LyricsRepository]:
    ...
    /** Disk budget for cached audio. [AudioCache][com.music.rizumu.playback.AudioCache] evicts past it. */
     ...
     * [ArtistFacts][com.music.rizumu.data.stats.ArtistFacts]. On by default,
     ...
     * [PlaybackService][com.music.rizumu.playback.PlaybackService] can pick
    ...
    fun init(context: Context) {
        prefs = context.getSharedPreferences("rizumu_settings", Context.MODE_PRIVATE)
     ...
     *
     * The one caller is an import ([Backup][com.music.rizumu.data.stats.Backup]),
    ...
    private fun readAll() {
        ...
        com.music.rizumu.data.lyrics.PaxSenix.setApiKey(paxSenixApiKey.value)
     ...
     *
     * Rizumu ships sideloaded (see [com.music.rizumu.data.AppUpdateChecker]),
    ...
    fun setPaxSenixApiKey(value: String) {
        val normalized = com.music.rizumu.data.lyrics.normalizePaxSenixApiKey(value)
        paxSenixApiKey.value = normalized
        prefs.edit().putString(KEY_PAXSENIX_API_KEY, normalized).apply()
        com.music.rizumu.data.lyrics.PaxSenix.setApiKey(normalized)
    }
     ...
     *
     * [Downloads][com.music.rizumu.download.Downloads] keeps its record of
