package com.music.rizumu.data.discord
 ...
 *
 * ```
 *   Listening to Rizumu          <- activityName, or the app's own name
 *   ┌────┐  Song title             <- details
 *   │art │  Artist                 <- state
 *   └────┘  ▁▁▁▁▁▁ 1:04 / 3:47     <- from the timestamps
 *   [ Listen on YouTube Music ]    <- button 1
 *   [ Visit Rizumu           ]   <- button 2
 * ```
 */
class DiscordRPC(
    val context: Context,
     ...
     * a side-by-side dev install should still look like Rizumu to everyone
     * else on Discord.
     */
    ...
    companion object {
        ...
        const val PROJECT_URL = "https://github.com/Sidharth-Singh10/Rizumu"

        const val DEFAULT_BUTTON_1 = "Listen on YouTube Music"
        const val DEFAULT_BUTTON_2 = "Visit Rizumu"
