package com.music.rizumu.data.sources.addon

import com.music.rizumu.data.Http
import com.music.rizumu.data.TrackLog
import com.music.rizumu.data.sources.module.SharedCalls
 ...
 * this side. Compare [ModuleManager][com.music.rizumu.data.sources.module.ModuleManager],
...
class AddonClient(rawBaseUrl: String) {
    ...
    suspend fun manifest(): Result<AddonManifest> = manifests.get(
        ...
        fetch<AddonManifest>(manifestUrl(baseUrl)).mapCatching { manifest ->
            ...
            if (!manifest.isPlayable) {
                ...
                throw AddonException("This addon declares $declared — Rizumu needs search")
            }
            manifest
        ...
        while (true) {
            ...
            val wait = client.newCall(request).execute().use { response ->
                when {
                    ...
                    attempt >= MAX_RETRIES -> throw AddonUnavailable("This addon is rate limiting Rizumu")
    ...
    companion object {
        ...
        /** The tiers [AddonSource][com.music.rizumu.data.sources.AddonSource] asks in. */
        ...
        private const val USER_AGENT = "Rizumu"
 ...
 * [SourceHealth.Rejected][com.music.rizumu.data.sources.SourceHealth.Rejected],
 ...
 * [SourceHealth][com.music.rizumu.data.sources.SourceHealth] draws.
 */
