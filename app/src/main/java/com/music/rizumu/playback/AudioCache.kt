 ...
 * original `rizumu://watch?v=<id>` request, and a cache hit never resolves a
 * URL at all.
 */
@UnstableApi
object AudioCache {
     ...
     * [StreamResolver][com.music.rizumu.data.innertube.StreamResolver]'s
     ...
     * `rizumu://watch?v=…` URI it builds for itself carries an id and nothing
     * else. It rides along from the queue instead — see
     * [PlaybackService.prefetchAround][com.music.rizumu.playback.PlaybackService].
     */
     ...
     * [PlaybackService][com.music.rizumu.playback.PlaybackService]'s audition.
    ...
    private suspend fun fetch(videoId: String, position: Long, length: Long) =
        fetch(videoId, Uri.parse("rizumu://watch?v=$videoId"), position, length)
